...
## Bắt xử lý ca quên tan ca

Nếu bản ghi mới nhất của 1 nhân viên là "Vào ca" và ngày đó **không phải hôm nay** (`getStaleOpenShift()` trong `js/app.js`) — nghĩa là công nhân đã quên bấm TAN CA hôm trước — app sẽ **chặn mọi thao tác khác**, bắt hiện màn hình riêng yêu cầu nhập giờ đã tan ca hôm đó trước khi cho tiếp tục dùng app (chỉ còn nút đăng xuất). Kiểm tra này chạy ở 1 cổng chung `routeAfterAuth()` — gọi từ lúc đăng nhập thành công, lúc mở app khi đã có phiên sẵn, và sau khi vừa xử lý xong tan ca bù.

**Cách ghi nhận tan ca bù:** không sửa/cập nhật lại bản ghi "Vào ca" gốc — thay vào đó tạo 1 bản ghi **"Tan ca" mới bình thường** (giờ = ngày vào ca gốc + giờ công nhân tự nhập, không cần chụp ảnh/GPS vì không còn ở công trình), đánh dấu cột **Ghi Chú** = `Bổ sung - quên tan ca`. Nhờ dùng chung luồng `api/checkin.js` + `api/payroll.js`, dữ liệu tự động khớp đúng vào **ngày gốc đã vào ca** (không phải ngày đang bổ sung) ở cả tab Lịch sử lẫn Bảng lương — không cần cơ chế "sửa lại" bản ghi cũ nào khác.
...
| Thời Gian | Số Điện Thoại | Mã Nhân Viên | Họ Tên | Loại | Dự Án | Vĩ Độ | Kinh Độ | Link Ảnh | Ghi Chú |
|---|---|---|---|---|---|---|---|---|---|

Cột **Ghi Chú** để trống với chấm công bình thường — chỉ có giá trị `Bổ sung - quên tan ca` cho các bản ghi "Tan ca" được tạo bù (xem mục "Bắt xử lý ca quên tan ca" bên dưới).
